...
- **Effort:** 0.5h
- **Done when:** `MAX_STALENESS_SECS = 30` в constants.rs, тесты обновлены (security properties test в `tests/holging.ts:369` проверяет `<= 120`, нужно `<= 30`).
...
- **Effort:** 0.5h
- **Done when:** `MAX_UPDATE_PRICE_DEVIATION_BPS = 1500` в constants.rs. Тест security properties (`tests/holging.ts:379`) обновлён.
...
- **Category:** Security
- **File(s):** Весь codebase (`programs/holging/src/`)
...
- **Effort:** 1h
- **Done when:** migrate_pool удалён из: instructions/migrate_pool.rs, instructions/mod.rs, lib.rs. IDL обновлён. Тест IDL validation (`tests/holging.ts:238`) обновлён (19 instructions вместо 20).
