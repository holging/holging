...
При базовом fee_bps = 4 и множителе ×5 при здоровом vault: стоимость раундтрипа = 0.40%. Для выхода в плюс необходимо:

$$
\frac{(x-1)^2}{2x} > 0.0008
$$

Приблизительно: SOL должен сдвинуться на ±9% для прибыли после комиссий.
...
Комиссия раундтрипа = 0.40%. Для малых движений SOL:
