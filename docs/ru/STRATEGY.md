...
- SOL должен сдвинуться на **±9%** чтобы покрыть комиссию 0.40% roundtrip
- При движении < ±9% стратегия в минусе на размер комиссии ($40 на $10K)
...
Комиссия: 0.40% roundtrip × размер ребалансировки
Максимум: 0.80% от портфеля (при полной перебалансировке обоих ног)
```
...
При этом пороге:
- Gain/fee ratio = 6.2x (комиссия = 16% от прибыли)
...
| Риск | Описание | Хедж |
|------|----------|------|
| **Малая волатильность** | SOL двигается < ±9%, комиссии > P&L | Выбрать период высокой волатильности |
...
| **Стоимость ребалансировки** | 0.80% от портфеля |
...
Где:
  ...
  rebalance_fee = 0.80%                      за каждую ребалансировку
