...
已在 Lean 4 中数学证明（8 个定理）。从经济角度 — SOL 变动 ±4% 时盈亏平衡（0.2% 往返手续费）。日常 1-2% 的波动，利润 ≈ 零。该策略从**波动率**中获利 — 波动越大 = 盈亏越大。这是没有时间衰减的永续跨式期权。

**问：如果 SOL 下跌 80% 怎么办？**
shortSOL 上涨 5 倍。金库必须支付 5 倍。熔断机制在金库比率 < 95% 时暂停。解决方案：超额抵押。公式：若要防护 -80%，金库 = 5x TVL。动态费率在压力下自动增至 0.5%，减缓资金流出。

### 经济模型

**问：谁是对手方？**
金库由资金池支撑。初始由团队/基金超额抵押。费用（0.1-0.5%）作为安全缓冲累积。高交易量下，金库自我维持。V2：外部 LP，收益共享。
...
**问：TAM/SAM/SOM？**
TAM：$47B（加密衍生品日交易量）。SAM：$2B（Solana 永续合约交易量）。SOM：$50M（一年目标，散户对冲 + holging）。按 0.2% 费率计算 = $50M 交易量下 $100K/年。
