# SolShort — Математика и Архитектура
...
Центральная идея SolShort — использование **реципрокной (обратной) функции** для создания хедж-инструмента.
...
**Выпуклость** (f'' > 0) — фундаментальное свойство. Именно оно создаёт положительную гамму и отличает SolShort от линейных inverse-инструментов.
...
**Мультипликативная модель (SolShort):**
...
SolShort — **зеркальное отражение** LP:
...
В отличие от TVL (Total Value Locked), SolShort оперирует понятием TVA — весь коллатерал, доступный для redemption, без lock-up периодов.
...
```
                        SolShort (Solana)         Shordex (EVM/Polygon)
  ────────────────────────────────────────────────────────────────────
...
*SolShort — Solafon Ecosystem · Built on Solana*
